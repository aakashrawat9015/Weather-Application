...
const EPA_BREAKPOINTS = [
  ...
  { iLow: 51, iHigh: 100, category: "Moderate", color: "bg-yellow-400/30 backdrop-blur-md", message: "Air quality is acceptable; some pollutants may be a concern for sensitive groups.", emoji: "🙂" },
  { iLow: 101, iHigh: 150, category: "Unhealthy for Sensitive Groups", color: "bg-orange-400/30 backdrop-blur-md", message: "Sensitive groups may experience health effects.", emoji: "😷" },
  { iLow: 151, iHigh: 200, category: "Unhealthy", color: "bg-red-500/30 backdrop-blur-md", message: "Everyone may begin to experience health effects; sensitive groups may experience more serious effects.", emoji: "⚠️" },
  { iLow: 201, iHigh: 300, category: "Very Unhealthy", color: "bg-purple-600/30 backdrop-blur-md", message: "Health alert: everyone may experience more serious health effects.", emoji: "☠️" },
  { iLow: 301, iHigh: 500, category: "Hazardous", color: "bg-pink-700/30 backdrop-blur-md", message: "Emergency conditions: entire population is likely to be affected.", emoji: "🚨" },
];

function getCategory(aqi) {
  if (aqi == null) return { category: "No Data", color: "bg-gray-400/30 backdrop-blur-md", message: "No AQI data available.", emoji: "❔" };
  for (let bp of EPA_BREAKPOINTS) {
    if (aqi >= bp.iLow && aqi <= bp.iHigh) return bp;
  }
  return { category: "Out of Range", color: "bg-gray-600/30 backdrop-blur-md", message: "AQI value is outside standard range.", emoji: "❔" };
}
